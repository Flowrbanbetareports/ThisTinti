...
from .ingestion import document_parse_error_detail, reprocess_document
...
def _automatic_document_intake(document: Document) -> dict[str, Any]:
    ...
    if document.parse_status == "failed":
        ...
        reason = str(detail.get("reason") or detail.get("message") or document.parse_message or "Interpretazione fallita")
        ...
        output.append(
            {
                ...
                "supplier": suppliers.get(document.supplier_id).legal_name if document.supplier_id in suppliers else None,
                "parse_status": document.parse_status,
                "confidence": float(document.confidence or 0),
                "file_available": Path(document.storage_path).is_file(),
                "created_at": document.created_at.isoformat(),
                "classification": classification,
                "can_retry": Path(document.storage_path).is_file() and document.parse_status in {"failed", "review_required"},
            }
        )
...
def classify_intake(
    ...
    *,
    ...
    exists = (
        ...
        else db.scalar(select(ProcessingJob.id).where(ProcessingJob.id == subject_id, ProcessingJob.tenant_id == tenant_id))
    )
    if not exists:
...
def case_rc15_payload(db: Session, tenant_id: str, case_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "allowed_actions": sorted(
            ...
            | ({"needs_review"} if "needs_review" in CASE_TRANSITIONS.get(item.status, set()) and item.status not in {"dismissed", "resolved"} else set())
        ),
        "evidence": [
            {
                "id": evidence.id,
...
def create_company_profile_version(
    ...
    max_version = int(
        db.scalar(select(func.coalesce(func.max(RC15CompanyProfileVersion.version), 0)).where(RC15CompanyProfileVersion.tenant_id == tenant_id))
        or 0
    )
    for current in db.scalars(
        select(RC15CompanyProfileVersion).where(
...
def ensure_company_profile(db: Session, tenant_id: str, user_id: str | None) -> RC15CompanyProfileVersion:
    ...
    if active is not None:
        return active
    active, _ = create_company_profile_version(db, tenant_id, user_id, "Profilo aziendale iniziale", DEFAULT_COMPANY_PROFILE)
    return active
...
def practice_payload(db: Session, tenant_id: str, item: RC15Practice) -> dict[str, Any]:
    ...
    ) if item.profile_version_id else None
...
def _pilot_case_payload(item: RC15PilotCase, *, include_ground_truth: bool) -> dict[str, Any]:
    ...
    if include_ground_truth:
        ...
        result["ground_truth_recorded"] = all(
            bool(_json(value, {}))
            for value in (item.reviewer_primary_json, item.reviewer_secondary_json, item.adjudicated_json)
        )
    return result


def pilot_payload(db: Session, tenant_id: str, item: RC15PilotWorkspace, *, include_ground_truth: bool = False) -> dict[str, Any]:
    cases = list(
        db.scalars(
    ...
    return {
        ...
        "measurement_complete_count": sum(
            case.manual_seconds is not None and case.assisted_seconds is not None and case.user_score is not None for case in cases
        ),
        "result": result,
...
def update_pilot_case(
    ...
    *,
    ...
    ground_truth_changes = any(value is not None for value in (reviewer_primary, reviewer_secondary, adjudicated))
    if ground_truth_changes and pilot.status != "draft":
        raise ValueError("La ground truth è congelata; crea una nuova versione del pilot per correggerla")
...
def freeze_pilot(db: Session, tenant_id: str, user_id: str | None, pilot_id: str) -> RC15PilotWorkspace:
    ...
    for item in cases:
        ...
        adjudicated = _json(item.adjudicated_json, {})
        if not primary or not secondary or not adjudicated:
            raise ValueError(f"Ground truth incompleta per la pratica {item.practice_id}")
    ...
    ) if pilot.profile_version_id else None
    manifest = {
...
def run_pilot(db: Session, tenant_id: str, user_id: str | None, pilot_id: str) -> RC15PilotWorkspace:
    ...
    for item in cases:
        ...
        expected = _expected_findings(frozen.get("adjudicated") or {})
        ...
        case_tp = sum(min(expected_counter[key], actual_counter[key]) for key in set(expected_counter) | set(actual_counter))
