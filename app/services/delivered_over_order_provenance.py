...
_RULE_ID = "builtin:delivered_over_order"
_RULE_VERSION = "1"
_RULE_CONFIGURATION_HASH = hashlib.sha256(
    json.dumps(
        {
            "commercial_role_priority": ["confirmation", "order", "proposal"],
            "comparison": "canonical_delivery_quantity_gt_canonical_commercial_quantity_plus_0.000001",
            "amount": "excess_canonical_quantity_times_weighted_canonical_commercial_unit_price",
            "support": (
...
_ROLES = ("proposal", "order", "confirmation", "delivery")
_NUMERIC_FIELDS = {"quantity", "unit_price", "price_base_quantity"}
_REQUIRED_COMMERCIAL_FIELDS = ("quantity", "unit_of_measure", "unit_price", "price_base_quantity")
_REQUIRED_DELIVERY_FIELDS = ("quantity", "unit_of_measure")
_FACT_TYPES = {
...
def _commercial_and_delivery_lines(
    db: Session,
    *,
    ...
    commercial = grouped[commercial_role].get(finding_key, [])
    deliveries = grouped["delivery"].get(finding_key, [])
    if not commercial or not deliveries:
        return None

    commercial_profile = quantity_profile(commercial)
    delivery_profile = quantity_profile(deliveries)
    if not profiles_compatible(commercial_profile, delivery_profile):
        return None
    if delivery_profile.quantity <= commercial_profile.quantity + _QTY_EPSILON:
        return None
    return commercial, deliveries
...
def _finding_key_for_case(db: Session, *, chain: OperationChain, case: DiscrepancyCase) -> str | None:
    ...
    if not documents[commercial_role] or not documents["delivery"]:
        return None
    grouped = group_chain_lines(db, chain, documents)
    keys = set(grouped[commercial_role]) | set(grouped["delivery"])
    matches: list[str] = []
    for key in sorted(keys):
        commercial = grouped[commercial_role].get(key, [])
        ...
        if delivery_profile.quantity <= commercial_profile.quantity + _QTY_EPSILON:
            continue
        fingerprint = hashlib.sha256(f"{chain.id}|delivered_over_order|{key}".encode()).hexdigest()
...
def _supporting_facts(
    ...
    *,
    ...
    lines = _commercial_and_delivery_lines(db, chain=chain, finding_key=finding_key)
    if lines is None:
        return []
    commercial, deliveries = lines
    document_ids = {line.document_id for line in [*commercial, *deliveries]}
    documents = {
    ...
    for line in sorted(commercial, key=lambda item: item.id):
        document = documents[line.document_id]
        for field_name in _REQUIRED_COMMERCIAL_FIELDS:
            fact = _direct_fact(db, chain=chain, line=line, document=document, field_name=field_name)
            if fact is None:
                return []
            facts.append(fact)
    for line in sorted(deliveries, key=lambda item: item.id):
        document = documents[line.document_id]
    ...
    commercial_profile = quantity_profile(commercial)
    delivery_profile = quantity_profile(deliveries)
    expected_amount = _money((delivery_profile.quantity - commercial_profile.quantity) * _weighted_price(commercial))
...
def delivered_over_order_finding_matches_current_support(
    db: Session,
    *,
    ...
    if case is None or case.tenant_id != finding.tenant_id or case.case_type != "delivered_over_order":
        return False
...
def record_delivered_over_order_finding_provenance(
    db: Session,
    *,
    chain: OperationChain,
    case: DiscrepancyCase,
