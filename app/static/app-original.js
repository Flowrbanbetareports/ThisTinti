...
const dateTime = (value) => value ? new Intl.DateTimeFormat('it-IT', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(value)) : '—';
...
async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(payload.detail || payload || `Errore ${response.status}`);
  return payload;
}
...
async function openValidationRun(runId) {
  try {
    ...
    $('#validationRunDialogBody').innerHTML = `<div class="detail-grid"><div class="detail-card"><p>Precisione</p><strong>${percentMetric(run.precision)}</strong></div><div class="detail-card"><p>Recall</p><strong>${percentMetric(run.recall)}</strong></div><div class="detail-card"><p>F1</p><strong>${percentMetric(run.f1_score)}</strong></div><div class="detail-card"><p>Errore importi</p><strong>${money(run.amount_mae)}</strong></div></div><div class="validation-scenarios"><h4>Scenari</h4>${scenarios.map(scenario => `<article class="validation-scenario ${scenario.passed ? 'passed' : 'failed'}"><div><strong>${escapeHtml(scenario.id)}</strong><small>${escapeHtml(scenario.description || '')}</small></div><span class="badge ${scenario.passed ? 'parsed' : 'high'}">${scenario.passed ? 'PASS' : 'FAIL'}</span><p>${scenario.true_positives || 0} TP · ${(scenario.false_positives || []).length} FP · ${(scenario.false_negatives || []).length} FN</p>${scenario.error ? `<code>${escapeHtml(scenario.error)}</code>` : ''}</article>`).join('')}</div>`;
