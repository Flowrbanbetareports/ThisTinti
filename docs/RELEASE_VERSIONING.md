...
From the first Qualified product release onward, ThisTinti uses ordinary stable SemVer tags:

- `v1.0.0` — first official Qualified release (`ThisTinti 1.0.0 Qualified — Procurement v1 — P1 — E1`);
- `v1.0.x` — qualified 1.0 maintenance line for security/critical fixes, with revalidation appropriate to each change;
- `v2.0.0` — Continuous Assurance product generation;
- `v3.0.0` — future commercialisation/distribution generation unless a later explicit product decision changes that boundary.

Existing `v3.4.0-alpha.*` / RC tags are immutable **legacy development prereleases**. They remain part of the repository history and must not be numerically compared with the new official line to infer chronology or readiness. Moving from a legacy `3.4.x` prerelease to official `1.0.0` is a versioning-boundary reset, not a downgrade.
...
5. publish `v1.0.0` only through the official-release path as a non-draft, non-prerelease release;
