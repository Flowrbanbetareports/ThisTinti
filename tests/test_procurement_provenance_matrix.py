...
def test_v01_baseline_is_preserved_as_historical_contract() -> None:
    assert json.loads(LEGACY_RULE_PACK_PATH.read_text(encoding="utf-8"))["schema"] == "thistinti.procurement-rule-pack.v1"
    assert json.loads(LEGACY_MATRIX_PATH.read_text(encoding="utf-8"))["schema"] == "thistinti.procurement-provenance-matrix.v1"
...
def test_blind_eligible_is_derived_from_scope_and_provenance() -> None:
    ...
    for rule in matrix["rules"]:
        assert rule["blind_eligible"] is (
            rule["blind_scope"] == "included" and rule["provenance_status"] == "complete"
        )
