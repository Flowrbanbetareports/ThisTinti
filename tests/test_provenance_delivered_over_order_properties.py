...
FIELD_STRATEGY = st.sampled_from(
    ("order_quantity", "order_uom", "order_unit_price", "order_price_base_quantity", "delivery_quantity", "delivery_uom")
)
...
def _stable_id(*parts: object) -> str:
    ...
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"thistinti-delivered-over-order-provenance:{key}"))
...
class _DeliveredScenario:
    def __init__(self, db: Session, *, token: str):
        self.db = db
        self.token = token
        self.tenant = Tenant(id=_stable_id(token, "tenant"), name=f"Delivered provenance {token}")
        self.user = User(
    ...
    def add_line_document(
        ...
        *,
        ...
        required = ("quantity", "unit_of_measure", "unit_price", "price_base_quantity") if role == "order" else (
            "quantity",
            "unit_of_measure",
        )
        locators = {
        ...
        for field in required:
            ...
            self._attach_fact(document=document, line=line, field=field, value=fields[field], kind=kind)
        return line

    def _attach_fact(self, *, document: Document, line: DocumentLine, field: str, value: object, kind: str) -> None:
        if kind == "missing":
            return
...
def test_property_delivered_over_order_complete_direct_support_binds_every_engine_input(
    ...
    with _isolated_db() as db:
        ...
        scenario.add_line_document(1, role="delivery", quantity=Decimal(ordered + excess))
        scenario.analyze()
...
def test_property_delivered_over_order_fails_closed_for_any_unqualified_required_input(
    ...
    elif source_field == "price_base_quantity":
        source_field = "price_base_quantity"
    if target == "order":
    ...
    with _isolated_db() as db:
        ...
        scenario.add_line_document(0, role="order", quantity=Decimal(ordered), kinds=order_kinds)
        scenario.add_line_document(
            1,
            role="delivery",
...
class DeliveredOverOrderProvenanceStateMachine(RuleBasedStateMachine):
    def __init__(self):
        ...
        self.scenario.add_line_document(0, role="order", quantity=Decimal("10"))
        self.scenario.add_line_document(1, role="delivery", quantity=Decimal("12"))
    ...
    def record_human_judgment(self) -> None:
        ...
        if judgment is not None:
            ...
            assert delivered_over_order_finding_matches_current_support(self.db, finding=latest) is True
        self._remember_history()
