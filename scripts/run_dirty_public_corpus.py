...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Characterize the raw external dirty-public corpus.")
...
def field_assertions(parsed: Any, expected: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for field in ("document_type", "number", "currency"):
        ...
        status = (
            ...
            else ("abstained" if observed in {None, "", "UNK"} else "wrong_non_null")
        )
...
def evaluate_parse(parsed: Any, expected: dict[str, Any]) -> list[str]:
    ...
    if document_type and parsed.document_type != document_type:
        failures.append(f"document_type expected {document_type!r}, got {parsed.document_type!r}")
...
def markdown(report: dict[str, Any]) -> str:
    ...
    failures = [
        item
        for item in report["cases"]
        if item["expectation_mode"] != "characterize" and not item["expectation_passed"]
    ]
    failure_lines = (
        "\n".join(
            f"- `{item['id']}`: " + "; ".join(item["expectation_failures"] or ["source assertion mismatch"])
...
def run_case(
    ...
    if outcome == "parse":
        ...
        if parse_error is None:
            result["expectation_failures"].append("expected structured ParseError rejection, but document parsed")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="thistinti-dirty-public-") as temp_dir:
        ...
        with httpx.Client(
            ...
            headers={"User-Agent": "ThisTinti-dirty-public-corpus/0.2"},
        ) as client:
    ...
    characterization_cases = [item for item in cases if item["expectation_mode"] == "characterize"]
    assertion_fields = [check for item in assertion_cases for check in item.get("field_assertions", [])]
