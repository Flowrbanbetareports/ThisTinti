...
def build(sources: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for index, (record, plan) in enumerate(zip(records, mutation_plan, strict=True), start=1):
        ...
        delivery = document(
            "delivery", f"DDT-PUB-{index:02d}", record["start_date"], supplier,
            sku, record["title"], 1, amount, 0, {"order_numbers": [ocid]},
        )
        invoice = document(
            "invoice", f"INV-PUB-{index:02d}", record["start_date"], supplier,
            sku, record["title"], 1, amount, 0,
        ...
        if mutation_type == "price_over_order":
            ...
            mutation = {"field": "invoice.lines[0].unit_price", "baseline": amount, "mutated": round(amount + float(value), 2)}
            ...
            mutation = {"field": "invoice.lines", "baseline_count": 1, "mutated_count": 2, "added_line_amount": float(value)}
    ...
    patterns = [
        ("clean", None), ("clean", None), ("price_over_order", 50.0),
        ("invoiced_over_received", None), ("discount_missing", None),
        ("unmatched_invoice_line", 75.0), ("return_without_credit", None),
        ("credit_below_return", None), ("price_over_order", 120.0), ("clean", None),
    ]
    base = date(2026, 8, 1)
    for index, (pattern, value) in enumerate(patterns, start=1):
        ...
        docs = [
            document("proposal", f"PROP-{index:03d}", str(base), supplier, sku, "Professional benchmark service", quantity, price, base_discount),
            document("order", order_no, str(base + timedelta(days=1)), supplier, sku, "Professional benchmark service", quantity, price, base_discount),
            document("confirmation", f"CONF-{index:03d}", str(base + timedelta(days=2)), supplier, sku, "Professional benchmark service", quantity, price, base_discount, {"order_numbers": [order_no]}),
        ]
        delivered = 8 if pattern == "invoiced_over_received" else quantity
        delivery = document("delivery", delivery_no, str(base + timedelta(days=3)), supplier, sku, "Professional benchmark service", delivered, price, base_discount, {"order_numbers": [order_no]})
        ...
        invoice = document("invoice", invoice_no, str(base + timedelta(days=4)), supplier, sku, "Professional benchmark service", quantity, invoice_price, invoice_discount, {"order_numbers": [order_no], "delivery_numbers": [delivery_no]})
        frozen_expected: list[dict[str, Any]] = []
        if pattern == "price_over_order":
            ...
            invoice["content"]["lines"].append({"line_no":2,"sku":f"EXTRA-{index:03d}","description":"Synthetic handling fee","quantity":1,"unit_price":float(value),"discount_rate":0})
            frozen_expected = expected("unmatched_invoice_line", float(value))
        docs.extend([delivery, invoice])
        invoice_total = sum(float(item["quantity"]) * float(item["unit_price"]) * (1 - float(item.get("discount_rate", 0)) / 100) for item in invoice["content"]["lines"])
        docs.append(document("payment", f"PAY-{index:03d}", str(base + timedelta(days=6)), supplier, f"PAY-{index:03d}", f"Payment for {invoice_no}", 1, invoice_total, 0, {"invoice_numbers": [invoice_no]}))
        if pattern == "return_without_credit":
            returned = 2
            docs.append(document("return", f"RET-{index:03d}", str(base + timedelta(days=7)), supplier, sku, "Professional benchmark service", returned, price, 0, {"order_numbers": [order_no], "invoice_numbers": [invoice_no]}))
            frozen_expected = expected("return_without_credit", returned * price)
        elif pattern == "credit_below_return":
            returned, credited = 3, 1
            docs.append(document("return", f"RET-{index:03d}", str(base + timedelta(days=7)), supplier, sku, "Professional benchmark service", returned, price, 0, {"order_numbers": [order_no], "invoice_numbers": [invoice_no]}))
            docs.append(document("credit_note", f"CN-{index:03d}", str(base + timedelta(days=9)), supplier, sku, "Professional benchmark service", credited, price, 0, {"order_numbers": [order_no], "invoice_numbers": [invoice_no]}))
    ...
    dataset = {
        ...
        "gate": {"min_precision":0.95,"min_recall":0.95,"min_f1":0.95,"max_amount_mae":0.05,"require_all_scenarios_pass":False},
        "scenarios": scenarios,
    }
    ground_truth = {
