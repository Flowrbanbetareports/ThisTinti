...
def _require_sha(value: Any, path: str, pattern: re.Pattern[str], *, allow_placeholder: bool) -> None:
...
def validate_manifest(data: dict[str, Any], *, final: bool = False) -> None:
    ...
    _require_sha(scope.get("sha256"), "p1_scope.sha256", SHA256, allow_placeholder=not final)
    ...
    _require_sha(candidate.get("source_sha"), "candidate.source_sha", SHA40, allow_placeholder=not final)
    ...
    for index, parser in enumerate(parsers):
        ...
        _require_string(parser_obj.get("id"), f"candidate.parser_set[{index}].id", allow_placeholder=not final)
        _require_string(
            parser_obj.get("version"), f"candidate.parser_set[{index}].version", allow_placeholder=not final
        )
        _require_sha(
    ...
    for component in ("rule_pack", "practice_model", "company_profile", "provenance_matrix"):
        ...
        _require_string(item.get("version"), f"candidate.{component}.version", allow_placeholder=not final)
        _require_sha(item.get("sha256"), f"candidate.{component}.sha256", SHA256, allow_placeholder=not final)
    ...
    for pool_name in POOLS:
        ...
        _require_string(pool.get("manifest_id"), f"pools.{pool_name}.manifest_id", allow_placeholder=not final)
        _require_sha(pool.get("sha256"), f"pools.{pool_name}.sha256", SHA256, allow_placeholder=not final)
    ...
    _require_string(reviewer.get("version"), "reviewer_protocol.version", allow_placeholder=not final)
    _require_sha(reviewer.get("sha256"), "reviewer_protocol.sha256", SHA256, allow_placeholder=not final)
